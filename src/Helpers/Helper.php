...
namespace Xmen\StarterKit\Helpers;
...
function logAdmin($method, $cls, $id)
{
    $act = explode('\\', $method);
...
function logAdminBatch($method, $cls, $ids)
{
    $act = explode('\\', $method);
    foreach ($ids as $id) {
        auth()->user()->logs()->create([
...
function gLog($cls, $id, $action, Request $request, $saveable = true)
{
    $c = GuestLog::whereDate('created_at', Carbon::today())->
    where('loggable_type', $cls)->
...
#                                   variables                                  #
    ...
    private $persian_short_month_names = ['', 'فرو', 'ارد', 'خرد', 'تیر', 'مرد', 'شهر', 'مهر', 'آ​با', 'آذر', 'دی', 'بهم', 'اسف'];
